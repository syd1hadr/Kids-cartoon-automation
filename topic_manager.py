...
def reserve_topic(
    ...
    source: str = "topic_manager",
) -> dict[str, Any]:
    topic = str(selected.get("topic", "")).strip()
    category = str(selected.get("category", "")).strip()

    if not topic or not category:
        raise RuntimeError(
            "reserve_topic ko valid topic aur category chahiye."
        )
    ...
    if already_exists:
        raise RuntimeError(
            f"Topic pehle hi history mein hai: {topic}"
        )
...
def choose_topic(
    ...
    suggested_normalized = normalize_topic(
        suggested_topic
    )
    ...
    if suggested_normalized:
        ...
        if (
            match
            and suggested_normalized
            not in blocked_topics
        ):
            return match.copy()
